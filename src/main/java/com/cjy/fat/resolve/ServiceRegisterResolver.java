...
public class ServiceRegisterResolver {
   ...
   public void registerService(FatServiceRegister txRegisterService ){
      ...
      if (serviceCount > 0) {
         ...
         // 将自己加入服务列表
         redisHelper.opsForServiceSetOperation().addToServiceSet(localTxKey, TransactionContent.getServiceId());
         ...
         redisHelper.opsForGroupKeySetOperation().addToGroupKeySet(TransactionContent.getRootTxKey(), localTxKey);
         // 加入到Group分组协调器的serviceSet中
         redisHelper.opsForGroupServiceSetOperation().addLocalServiceSetToGroupServiceSet(TransactionContent.getLocalTxKey(), TransactionContent.getRootTxKey());
      }
      // 生成本地事务标识
      for(int i = 0 ; i < localTransactionCount ;i ++) {
         ...
         if(StringUtils.isNotBlank(TransactionContent.getLocalTxKey())) {//当产生了localTxKey，需要继续调用服务的时候，需要把mark加入redis作为cancomlist的元素serviceSet
            // 加入服务列表 ，增加需要完成的服务数量
            redisHelper.opsForServiceSetOperation().addToServiceSet(TransactionContent.getLocalTxKey(), localTxMark);
         }
         //存在远程txKey，被调用
         if(StringUtils.isNotBlank(TransactionContent.getRemoteTxKey())) {
            ...
            if(StringUtils.isBlank(TransactionContent.getLocalTxKey())) {
               redisHelper.opsForServiceSetOperation().addToServiceSet(TransactionContent.getRemoteTxKey(), localTxMark);
            }
         }
